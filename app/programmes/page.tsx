...
import { getCourses } from "../cms/courses";
...
export default async function ProgrammesPage() {
  ...
  const accreditedCourses = courses.filter((course) => course.category === "accredited");
  const flagshipCourse = courses.find((course) => course.category === "flagship");
  const shortCourses = courses.filter((course) => course.category === "short-course");
    ...
    <main>
        ...
        <div className="shell">
          ...
          <div className="qualification-grid">
            {accreditedCourses.map((course, index) => (
              <article className="qualification-card" key={course.id}>
                <div className="qualification-top"><span>{String(index + 1).padStart(2, "0")}</span><strong>{course.duration}</strong></div>
                <p>SAQA qualification {course.saqaId ?? "Confirmed per intake"}</p>
                <h3>{course.title}</h3>
                <div className="qualification-bottom"><span>{course.nqfLevel ?? "Accredited programme"}</span><Link href={`/contact?service=${encodeURIComponent(course.title)}#enquiry`}>Programme enquiry ↗</Link></div>
              </article>
            ))}
          </div>
      ...
      <section className="flagship-section" id="azibuye">
        <div className="shell flagship-grid">
          ...
          <div className="flagship-copy"><p className="eyebrow dark"><span />{flagshipCourse?.title ?? "Azibuye Emasisweni"}</p><h2>100 youth and family poultry entrepreneurs.</h2><p className="lead">{flagshipCourse?.summary ?? "A programme for unemployed youth and families that moves participants from foundational learning to small-scale business ownership."}</p><p>The model combines practical poultry production, housing, feeding, biosecurity, recordkeeping, budgeting, financial management and business planning with starter guidance and post-training mentorship.</p><dl className="programme-facts"><div><dt>Target</dt><dd>{flagshipCourse?.capacity ?? 100} beneficiaries</dd></div><div><dt>Cohorts</dt><dd>5 groups of 20</dd></div><div><dt>Journey</dt><dd>{flagshipCourse?.duration ?? "10 training days"}</dd></div><div><dt>Support</dt><dd>Mentorship & monitoring</dd></div></dl><Link href={`/contact?service=${encodeURIComponent(flagshipCourse?.title ?? "Azibuye Emasisweni")}#enquiry`} className="button button-dark">Fund or host this programme <span>↗</span></Link></div>
        </div>
      </section>
      ...
      <section className="section packaged-programmes">
        <div className="shell"><div className="section-heading"><div><p className="eyebrow dark"><span />Other packaged programmes</p><h2>Ready to adapt to your context.</h2></div></div><div className="programme-list">{shortCourses.map((course, i) => <article key={course.id}><span>{String(i + 1).padStart(2, "0")}</span><h3>{course.title}</h3><p>{course.summary}</p><Link href={`/contact?service=${encodeURIComponent(course.title)}#enquiry`}>Enquire ↗</Link></article>)}</div></div>
      </section>
