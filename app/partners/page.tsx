...
export default function PartnersPage() {
  return (
    <main>
      ...
      <section className="section clients-section"><div className="shell"><div className="section-heading"><div><p className="eyebrow dark"><span />Who we work with</p><h2>Programmes shaped around public and community priorities.</h2></div></div><div className="client-grid">{clients.map(([title, text], i) => <article key={title}><span>{String(i + 1).padStart(2, "0")}</span><h3>{title}</h3><p>{text}</p></article>)}</div></div></section>
